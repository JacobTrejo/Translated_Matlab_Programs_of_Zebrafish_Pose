...
from Translated_Python_Programs_Without_Randoms import view_s_lut_new_real_cpu_NORANDOM, view_b_lut_new_real_cpu_NORANDOM, return_head_real_model_new_NORANDOMS

def roundHalfUp(a):
    return (np.floor(a)+ np.round(a - (np.floor(a)-1))-1)

def return_graymodels_fish(x,lut_b_tail,lut_s_tail,proj_params,fishlen,imageSizeX,imageSizeY,r):
    #Just assigning some random values
    r1 = r[0]
    r2 = r[1]
    r3 = r[2]
    r4 = r[3]
    ...
    # SHOULD DOUBLE CHECK THE FORMAT
    ...
    crop_b[0] = roundHalfUp(coor_b[1,2]) - (imageSizeY - 1)/2 + roundHalfUp((r1 - 0.5)*40)
    crop_b[1] = crop_b[0] + imageSizeY - 1
    crop_b[2] = roundHalfUp(coor_b[0,2]) - (imageSizeX - 1)/2 + roundHalfUp((r2 - 0.5)*40)
    crop_b[3] = crop_b[2] + imageSizeX - 1



    crop_s1 = np.array([0.0,0.0,0.0,0.0])
    crop_s1[0] = roundHalfUp(coor_s1[1,2]) - (imageSizeY - 1)/2 + roundHalfUp((r3 - 0.5)*40)
    crop_s1[1] = crop_s1[0] + imageSizeY - 1
    crop_s1[2] = roundHalfUp(coor_s1[0,2]) - (imageSizeX - 1)/2 + roundHalfUp((r4 - 0.5)*40)
    crop_s1[3] = crop_s1[2] + imageSizeX - 1

    crop_s2 = np.array([0.0,0.0,0.0,0.0])
    crop_s2[0] = roundHalfUp(coor_s2[1,2]) - (imageSizeY - 1)/2 + roundHalfUp((r5 - 0.5)*40)
    crop_s2[1] = crop_s2[0] + imageSizeY - 1
    crop_s2[2] = roundHalfUp(coor_s2[0,2]) - (imageSizeX - 1)/2 + roundHalfUp((r6 - 0.5)*40)
    crop_s2[3] = crop_s2[2] + imageSizeX - 1

    (projection_b,projection_s1,projection_s2,eye_b,eye_s1,eye_s2,eye_coor_3d) = return_head_real_model_new_NORANDOMS.return_head_real_model_new(x, fishlen, proj_params, crop_b, crop_s1, crop_s2, r1, r2, r3, r4, r5)

    temp = np.copy(coor_b)
    gray_b = view_b_lut_new_real_cpu_NORANDOM.view_b_lut_new_real_cpu(crop_b, temp, lut_b_tail, projection_b, imageSizeX, imageSizeY, r1, r2)
    temp = np.copy(coor_s1)
    gray_s1 = view_s_lut_new_real_cpu_NORANDOM.view_s_lut_new_real_cpu(crop_s1, temp, lut_s_tail, projection_s1, imageSizeX, imageSizeY, r3, r4)
    temp = np.copy(coor_s2)
    gray_s2 = view_s_lut_new_real_cpu_NORANDOM.view_s_lut_new_real_cpu(crop_s2, temp, lut_s_tail, projection_s2, imageSizeX, imageSizeY, r5, r6)
